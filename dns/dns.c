...
int ig_trace_dns(struct __sk_buff *skb)
{
  ...
  */
  ...
  event.answer_count = ancount;
  event.authority_count = nscount;
  ...
  if (ancount > 0) {
    // Answers are a resource record after the question section that was just parsed.
    // After QNAME (len) is 2 bytes of QTYPE and 2 bytes of QCLASS.
    // https://datatracker.ietf.org/doc/html/rfc1035#section-4.1.3
    unsigned int answer_off = DNS_OFF + sizeof(struct dnshdr) + len + 4;

    // TTL is after the NAME again then TYPE + CLASS which are each 2 bytes.
    event.ans_ttl = load_word(skb, answer_off + len + 2 + 2);
    // ipv6 mad hacks... sorry.
    if (event.qtype == 28) {
      // TODO make this work.
      /*
      for (i = 0; i < 4; i = i+4) {
        event.ans_addr_v6[i] = load_word(skb, answer_off + len + 2 + 2 + 4 + 4 + i);
        event.ans_addr_v6[i] = bpf_htonl(event.ans_addr_v6[i]);
      }
      */
      // bpf_probe_read(event.ans_addr_v6, sizeof(event.ans_addr_v6), (void *)skb->data + answer_off + len + 2 + 2 + 4 + 4);
    } else {
      // Ignore RDLENGTH and just read RDATA, assuming it is an ipv4 address.
      event.ans_addr_v4 = load_word(skb, answer_off + len + 2 + 2 + 4 + 4);
      event.ans_addr_v4 = bpf_htonl(event.ans_addr_v4);
    }
  }
